...
```shell
# 查看prefix路径，-g选项显示全局模式下的路径
npm prefix [-g]
```

在使用 npm 安装包的时候，有两种模式：**本地模式**和**全局模式**：
...
2. 全局模式
    ...
    在全局模式下，Unix 系统中包安装到`{prefix}/lib/node_modules`，Windows 系统中的安装到`{prefix}/node_modules`。可执行文件在 Unix 中链接到`{prefix}/bin`，在Windows 中直接链接到`{prefix}`。
...
- `main`
  ...
  这个字段指定了包的主要入口，默认值是在包根目录下的`index.js`。

> 详细配置项如参考：[package.json配置项](https://docs.npmjs.com/cli/v9/configuring-npm/package-json)。
...
```shell
# 创建package.json文件，-y表示快速跳过，从当前目录提取信息
npm init [-y]
# 创建scoped包
...
```shell
# 安装所有在package.json中的依赖
npm install
# 安装指定的包，scope表示作用域，tag默认为latest
npm install [<@scope>/]<name>
npm install [<@scope>/]<name>@<tag>
npm install [<@scope>/]<name>@<version>
# 查看已安装的包，-g表示以全局模式安装的包
npm list [-g]
# 卸载包，-g表示全局模式
...
使用`npm ci`命令可以确保纯净的安装，使用`npm install`和`npm ci`的主要区别是：
...
- `-O, --save-optional`
    ...
    安装到`optionalDependencies`项中。当希望某些依赖即使下载失败或者没有找到时，项目依然可以正常运行或者npm继续运行，就可以把这些依赖放在`optionalDependencies`对象中。

- `--no-save`

    防止保存到`dependencies`项中。
...
有一些特殊的生命周期脚本只在某些情况下发生，这些脚本是在`pre<event>`、`post<event>`和`<event>`脚本之外发生的，详细参考[生命周期脚本](https://docs.npmjs.com/cli/v9/using-npm/scripts#life-cycle-scripts)。

- `prepare`
- `prepublishOnly`
- `prepack`
- `postpack`
- `dependencies`：不在全局模式下运行。每当 npm 命令导致`node_modules`目录发生更改时，就会运行依赖项脚本。它在应用更改并更新`package.json`和`package-lock.json`文件之后运行。

---

生命周期规则参考[生命周期操作规则](https://docs.npmjs.com/cli/v9/using-npm/scripts#life-cycle-operation-order)，部分常用命令如下：
...
- `npm start`
    ...
    如果包根目录中有一个 server.js 文件，那么 npm 会默认 start 命令到`node server.js`。在这种情况下，prestart 和 poststart 仍将运行。
...
# 在所有的workspace中运行脚本，--if-present表示会忽略不存在的脚本
...
`.pnpm/`以平铺的形式储存着所有的包，所以每个包都可以在这种命名模式的文件夹中被找到，这被称之为虚拟存储目录。

```
.pnpm/<name>@<version>/node_modules/<name>
```

 `pnpm install`用于安装所有的依赖项，在 CI 环境中, 如果存在锁文件但需要更新，那么安装会失败。

```shell
pnpm install --frozen-lockfile
```

`--frozen-lockfile`选项对于非 CI 环境，这个选项默认值为`false`。对于 CI 环境，如果存在锁文件，则默认为`true`。`--frozen-lockfile`选项如果设置为`true`, pnpm 不会生成锁文件，且如果锁文件跟清单文件不同步、锁文件需要更新或不存在锁文件，则会安装失败。
